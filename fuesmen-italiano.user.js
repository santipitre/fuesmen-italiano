...
(function () {
  ...
  function buildHTML(P){
    ...
    +'<button class="btn x" id="bx">⬇ Exportar Excel</button><button class="btn c" id="bc">⬇ CSV</button>'
    ...
    +'<div class="wrap"><table id="t"><thead><tr><th>#</th><th>N° Pedido</th><th>Documento</th><th>Paciente</th><th>Fecha y hora</th><th>Tipo / Ubicación</th><th>O.S. / Aseguradora</th><th>Prestador</th><th>Turno(s)</th><th>Estudios solicitados</th><th>Diagnóstico</th><th>Observación</th></tr></thead><tbody>'+rh+'</tbody></table></div>'
    ...
    +'function stamp(){var d=new Date(),p=function(n){return(n<10?"0":"")+n;};return d.getFullYear()+"-"+p(d.getMonth()+1)+"-"+p(d.getDate())+"_"+p(d.getHours())+p(d.getMinutes());}'
    +'function flat(){var out=[];DATA.forEach(function(p,i){var base={"#":i+1,"N° Pedido":p.pedidoNro,"Doc":p.docTipo,"Documento":p.doc,"Paciente":p.nombre,"Tipo":p.tipo||"","Ubicación":p.ubicacion||"","Servicio":p.servicio||"","O.S.":p.os||"","Aseguradora":p.aseguradora||"","Prestador":p.prestador,"Fecha y hora":p.fecha,"Estado turno":p.estadoTurno,"Estudios solicitados":(p.codigos||[]).join(" | "),"Diagnóstico":p.diagnostico||"","Observación":p.observacion||""};'
    ...
    +'var COLS=["#","N° Pedido","Doc","Documento","Paciente","Tipo","Ubicación","Servicio","O.S.","Aseguradora","Prestador","Fecha y hora","Estado turno","Turno N°","Turno Fecha","Turno Estudio","Estudios solicitados","Diagnóstico","Observación"];'
