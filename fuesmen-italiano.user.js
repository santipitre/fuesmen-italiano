...
// @version      5.13
// @description  Muestra el N de turno de FUESMEN al lado de cada pedido del Italiano y lo carga en "Numero de informe". Usa GM_xmlhttpRequest para saltar el bloqueo de Chrome (Private Network Access).
...
(function () {
  ...
  // ---- transporte privilegiado (evita el bloqueo PNA de Chrome) ----
  function gmGet(url, ok, fail) {
    GM_xmlhttpRequest({ method:'GET', url:url, onload:function(r){ try{ ok(JSON.parse(r.responseText)); }catch(e){ fail&&fail(e); } }, onerror:function(e){ fail&&fail(e); } });
  }
  function gmPost(url, obj) {
    try { GM_xmlhttpRequest({ method:'POST', url:url, headers:{'Content-Type':'application/json'}, data:JSON.stringify(obj) }); } catch(e){}
  }
  ...
  function fillMode(input){
    ...
    if(g && auto){
      // El usuario pidio: al apretar Cargar, GUARDAR automaticamente.
      sessionStorage.setItem('fuesmen_justsaved','1');   // al volver a la lista, saltar al proximo verde
      toast('N° '+turno+' pegado'+(meta?(' - '+meta):'')+'. Guardando…','#2ea043');
      setTimeout(function(){ g.click(); try{ if(g.form && g.form.requestSubmit) g.form.requestSubmit(g); }catch(e){} }, 300);
    } else {
      toast('N de turno '+turno+' pegado'+(meta?(' - '+meta):'')+(g?'. Revisa y apreta GUARDAR.':'. No encontre GUARDAR; apretalo vos.'),'#2ea043');
    }
  }

  // ---- Set de pedidos ya cargados (para marcar ✓ y autoavanzar) ----
  var DONE={}; try{ (JSON.parse(localStorage.getItem('fuesmen_done')||'[]')||[]).forEach(function(id){ DONE[id]=1; }); }catch(e){}
  function doneSave(){ try{ localStorage.setItem('fuesmen_done', JSON.stringify(Object.keys(DONE))); }catch(e){} }
  function markDone(id){ if(id){ DONE[id]=1; doneSave(); } }

  // ---- Panel lateral izquierdo: bolsa VERDE (1 turno) y bolsa NARANJA (varios) ----
  var SIDE_REG={};   // pedidoId -> { id, kind:'green'|'orange', dni, paciente, turno, fecha, info, cands, sel, load() }
  function rectTop(info){ try{ return info.tr.getBoundingClientRect().top + window.scrollY; }catch(e){ return 0; } }
  // clave de orden por fecha del pedido (dd/mm/yyyy) -> número comparable; faltante = al final
  function dkey(s){ var m=(s||'').match(/(\d{2})\D(\d{2})\D(\d{4})/); return m ? (+m[3]*10000 + (+m[2])*100 + (+m[1])) : 99999999; }
  // Localiza la fila del pedido EN VIVO por pedidoId (GeneXus re-dibuja la lista y deja huérfanas las refs viejas).
  function liveRow(e){
    if(!e) return null;
    if(e.id){
      var all=document.querySelectorAll('[onclick*="informe('+e.id+')"]');
      for(var i=0;i<all.length;i++){ if(all[i].offsetParent!==null) return all[i].closest('tr')||pedidoRow(all[i]); } // preferir la VISIBLE
  ...
  // Click en el DNI: scrollea al primer pedido de ese DNI y resalta TODOS sus recuadros en B (azul).
  ...
  function injectSidebar(){
    if(document.getElementById('fm-side')) return;
    var s=document.createElement('div'); s.id='fm-side';
    s.style.cssText='position:fixed;left:6px;top:84px;bottom:10px;width:248px;z-index:99990;display:flex;flex-direction:column;gap:8px;font-family:Segoe UI,sans-serif';
    var tg=document.createElement('button'); tg.id='fm-side-toggle';
    tg.style.cssText='font:700 11px Segoe UI;color:#fff;background:#444c56;border:0;padding:5px 9px;border-radius:6px;cursor:pointer;align-self:flex-start;box-shadow:0 2px 6px rgba(0,0,0,.25)';
    function setHidden(h){ var gb=document.getElementById('fm-side-green'),ob=document.getElementById('fm-side-orange'); if(gb)gb.style.display=h?'none':''; if(ob)ob.style.display=h?'none':''; tg.textContent=h?'▸ Mostrar listas':'◂ Ocultar listas'; try{localStorage.setItem('fuesmen_side_hidden',h?'1':'0');}catch(e){} }
    tg.onclick=function(){ var vis=document.getElementById('fm-side-green').style.display!=='none'; setHidden(vis); };
    function boxEl(id,headColor,bg,border){
      var b=document.createElement('div'); b.id=id;
      b.style.cssText='flex:1;min-height:64px;display:flex;flex-direction:column;background:'+bg+';border:2px solid '+border+';border-radius:10px;overflow:hidden;box-shadow:0 3px 10px rgba(0,0,0,.18)';
      var h=document.createElement('div'); h.id=id+'-h'; h.style.cssText='font:800 12px Segoe UI;color:#fff;background:'+headColor+';padding:7px 10px;display:flex;justify-content:space-between;align-items:center;cursor:pointer';
      h.title='Click en el encabezado: ir al próximo pendiente';
      var list=document.createElement('div'); list.id=id+'-list'; list.style.cssText='overflow:auto;padding:6px 8px;flex:1';
      b.appendChild(h); b.appendChild(list); return b;
    }
    s.appendChild(tg);
    s.appendChild(boxEl('fm-side-green','#1a7f37','#eafbf0','#2ea043'));
    s.appendChild(boxEl('fm-side-orange','#9a6700','#fff8e5','#e0a106'));
    document.body.appendChild(s);
    document.getElementById('fm-side-green-h').onclick=nextPendingGreen;
    var hidden=false; try{ hidden=localStorage.getItem('fuesmen_side_hidden')==='1'; }catch(e){}
    setHidden(hidden);
  }
  function renderSidebar(){
    var gC=document.getElementById('fm-side-green-list'), oC=document.getElementById('fm-side-orange-list');
    if(!gC||!oC) return;
    function build(kind, cont){
      cont.innerHTML='';
      var entries=Object.keys(SIDE_REG).map(function(id){ return SIDE_REG[id]; })
        .filter(function(e){ return e.kind===kind; })
        .sort(function(a,b){ return dkey(a.fecha)-dkey(b.fecha); });   // fecha más antigua primero
      var groups={}, order=[];
      entries.forEach(function(e){ var d=e.dni||'?'; if(!groups[d]){ groups[d]=[]; order.push(d); } groups[d].push(e); });
      var pend=0;
      order.forEach(function(d){
        var arr=groups[d];
        var g=document.createElement('div'); g.style.cssText='margin-bottom:6px;border-bottom:1px solid rgba(0,0,0,.08);padding-bottom:5px';
        var head=document.createElement('div'); head.style.cssText='font:700 12px Segoe UI;color:#1f2328;cursor:pointer';
        head.title='Resaltar este DNI en el sistema B e ir hasta ahí';
        head.textContent='DNI '+d+(arr.length>1?(' · '+arr.length):'');
        (function(a){ head.onclick=function(){ scrollToDni(a); }; })(arr);
        if(arr[0].paciente){ var pn=document.createElement('span'); pn.style.cssText='display:block;font:500 11px Segoe UI;color:#57606a;white-space:nowrap;overflow:hidden;text-overflow:ellipsis'; pn.textContent=arr[0].paciente; head.appendChild(pn); }
        g.appendChild(head);
        arr.forEach(function(e){
          var done=!!DONE[e.id]; if(!done) pend++;
          var row=document.createElement('div'); row.style.cssText='display:flex;align-items:center;gap:6px;margin-top:3px;'+(done?'opacity:.45':'');
          var lab=document.createElement('span');
          lab.style.cssText='flex:1;font:700 12px Segoe UI;cursor:pointer;'+(kind==='green'?'color:#0a5c28':'color:#8a5a00');
          lab.textContent=(done?'✓ ':'')+(kind==='green'? e.turno : ((e.cands?e.cands.length:'?')+' turnos'));
          lab.title='Ir al pedido'; lab.onclick=function(){ scrollToPedido(e,true); };
          row.appendChild(lab);
          if(e.fecha){ var fc=document.createElement('span'); fc.style.cssText='font:500 10px Segoe UI;color:#8b949e;white-space:nowrap'; fc.textContent=e.fecha.slice(0,5); row.appendChild(fc); }
          if(!done){
            var b=document.createElement('button'); b.textContent='Cargar';
            b.style.cssText='font:700 11px Segoe UI;color:#fff;border:0;padding:3px 9px;border-radius:6px;cursor:pointer;background:'+(kind==='green'?'#1f6feb':'#9a6700');
            b.onclick=function(ev){ ev.preventDefault(); e.load(); };
            row.appendChild(b);
          }
          g.appendChild(row);
        });
      ...
      if(!order.length) cont.innerHTML='<div style="font:500 12px Segoe UI;color:#8b949e;padding:6px">— ninguno —</div>';
      return pend;
    }
    var pg=build('green',gC), po=build('orange',oC);
    var gh=document.getElementById('fm-side-green-h'); if(gh) gh.innerHTML='<span>🟢 1 turno</span><span>'+pg+' pend. ▶</span>';
    var oh=document.getElementById('fm-side-orange-h'); if(oh) oh.innerHTML='<span>🟠 Varios turnos</span><span>'+po+' pend.</span>';
  }
  ...
  function loadInforme(info,turno,metaTxt){
    ...
    sessionStorage.setItem('fuesmen_autosave','1');   // el usuario pidio: al apretar Cargar, Guardar solo
    gmPost(SERVER+'/api/logcarga',{ turno:turno, pedidoId:info.pedidoId, dni:info.dni, paciente:info.paciente, estudio:info.estudio });
    info.anchor.click();
  }

  // ===== CARGA EN LOTE de casos SEGUROS =====
  // Seguro = trámite completo (sin bandera roja) + turno ÚNICO + Nº Referencia FUESMEN == "Pedido N°" que muestra B.
  var SAFE_MAP={};   // pedidoId -> { pedidoId, turno, meta, dni, paciente, pedido, fecha }
  function safeList(){ return Object.keys(SAFE_MAP).map(function(k){ return SAFE_MAP[k]; }); }
  // Índice Nº Referencia (PedidoMed) -> turno, SOLO si ese Nº mapea a un único turno.
  var REFMAP={};
  ...
  // Caso SEGURO = "Pedido N°" de B == Nº Referencia FUESMEN (ÚNICO) + trámite completo. (No exige turno único del DNI: el Nº de pedido ya identifica el turno 1:1.)
  function scanSafe(){
    SAFE_MAP={};
    [].slice.call(document.querySelectorAll('[onclick*="informe("]')).forEach(function(a){
      ...
      if(REVISAR[mId[1]]) return;   // un pedido marcado "a revisar" NUNCA entra en la carga automática
  ...
  function showSafeModal(){
    ...
    if(!list.length){ body.innerHTML='<div style="color:#57606a;font:500 13px Segoe UI;padding:10px">No hay casos que cumplan las 3 condiciones (trámite completo + turno único + Nº Referencia = Pedido N° de B).</div>'; }
    else{
      ...
      hr.innerHTML='<th style="padding:4px">Fecha</th><th>DNI</th><th>Paciente</th><th>Turno</th><th>Pedido N°</th>'; tbl.appendChild(hr);
      list.forEach(function(s){ var tr=document.createElement('tr'); tr.style.cssText='border-bottom:1px solid #f0f0f0';
        tr.innerHTML='<td style="padding:4px;color:#57606a">'+(s.fecha||'')+'</td><td>'+s.dni+'</td><td style="max-width:190px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">'+(s.paciente||'')+'</td><td style="color:#0a5c28;font-weight:800">'+s.turno+'</td><td style="color:#1f6feb;font-weight:700">'+s.pedido+'</td>';
        tbl.appendChild(tr); });
    ...
    var warn=document.createElement('span'); warn.style.cssText='flex:1;font:600 11px Segoe UI;color:#9a6700'; warn.textContent='Carga y Guarda cada uno automáticamente, en secuencia.';
    ...
    test.title='Carga solo los primeros 3 para que verifiques antes de soltar todo';
    test.onclick=function(){ ov.remove(); startBatch(list.slice(0,3)); };
    var go=document.createElement('button'); go.textContent='⚡ Cargar todos ('+list.length+')';
    go.style.cssText='font:800 14px Segoe UI;color:#fff;background:'+(list.length?'#1a7f37':'#6e7781')+';border:0;padding:10px 18px;border-radius:9px;cursor:'+(list.length?'pointer':'default'); go.disabled=!list.length;
    go.onclick=function(){ ov.remove(); startBatch(list); };
    foot.appendChild(warn); foot.appendChild(test); foot.appendChild(go);
  ...
  function startBatch(list){
    ...
    updateSafeBtn();
    toast('Cargando '+q.length+' casos seguros, uno tras otro…','#1a7f37');
    processQueue();
  }
  var QUEUE_TRIES=0;
  function processQueue(){
    ...
    if(!a){ // el pedido todavía no se dibujó: esperar; tras varios intentos, descartarlo
      if(QUEUE_TRIES++ < 20){ setTimeout(processQueue, 500); return; }
      q.shift(); try{ localStorage.setItem('fuesmen_queue', JSON.stringify(q)); }catch(e){} QUEUE_TRIES=0; setTimeout(processQueue, 300); return;
    }
    QUEUE_TRIES=0;
    q.shift(); try{ localStorage.setItem('fuesmen_queue', JSON.stringify(q)); }catch(e){}   // sacar antes de navegar
    ...
    gmPost(SERVER+'/api/logcarga',{ turno:item.turno, pedidoId:item.pedidoId });
    toast('Lote: turno '+item.turno+' ('+q.length+' restantes)…','#1f6feb');
    a.click();   // -> Carga informe -> fillMode auto-Guarda -> vuelve a la lista -> processQueue sigue
  }

  // ---- helpers para abrir FUESMEN desde el panel de B (reusa la ventana 'fuesmenHIS') ----
  ...
  // ---- Vistas: TODOS / solo con turno / a revisar ----
  ...
  // Marca visualmente una fila "a revisar" (borde + fondo ámbar + chip). Idempotente y reversible.
  ...
  function applyView(announce){
    ...
    anchors.forEach(function(a){
      ...
      else { show = true; }   // TODOS = realmente todos, incluidos los "a revisar"
      row.style.display = show ? '' : 'none';
      // En la vista TODOS, los "a revisar" se ven Y quedan marcados. En la vista exclusiva
      // "A revisar" no hace falta marcar (todo lo visible ya es a revisar).
  ...
  function injectToggle(){
    ...
    bar.style.cssText='position:fixed;top:12px;right:16px;z-index:99999;display:flex;gap:8px';
    ...
    bar.appendChild(b4); bar.appendChild(b5); bar.appendChild(b1); bar.appendChild(b2); bar.appendChild(b3); document.body.appendChild(bar);
  ...
  function annotate(){
    ...
    [].slice.call(document.querySelectorAll('[onclick*="informe("]')).forEach(function(a){
      ...
      if(a.offsetParent===null) return;   // ancla OCULTA (display:none, ~1421 en la pág): no anotar ni marcar; se reintenta si se muestra
      a.dataset.fmDone='1';
      var info=parseRow(a); if(!info||!info.dni) return;
      var cands=bestForRow(info.dni,info.estudio);
      if(!cands.length) return;   // DNI no esta en la worklist del mes activo -> no anotar (sin ruido y mas rapido)
      ...
      function cargarBtn(color,fn){ var b=document.createElement('button'); b.textContent='Cargar'; b.style.cssText=btnCss(color)+';padding:7px 16px;font-size:13px'; b.onclick=function(ev){ ev.preventDefault(); fn(); }; return b; }
      {
        ...
        if(strong){
          ...
          if(info.pedidoId){ SIDE_REG[info.pedidoId]={ id:info.pedidoId, kind:'green', dni:info.dni, paciente:info.paciente, turno:t.turno, fecha:info.fechaPedido, info:info,
            load:(function(tt,mm){ return function(){ loadInforme(info,tt,mm); }; })(t.turno, t.practicas[0]||'') }; }
        } else {
      ...
      // Ubicar el panel en la zona ancha del pedido (celda "Detalle de pedido"), no en la angosta de "Accion".
      var cells=[].slice.call(info.tr.querySelectorAll('td'));
      // Dia de la semana en la celda de "Fecha y hora"
      ...
      // DNI clickable -> buscar por documento en FUESMEN (celda Prestador)
      ...
      // N de pedido medico (del onclick prt(NNN) de la impresora) en la celda de Accion
    ...
    applyView();   // mantener la vista activa (todos / con turno / a revisar) al re-anotar
    scanSafe();
  }

  // ---- Buscador dentro de la pagina de B (filtra los pedidos por DNI o apellido) ----
  function filterB(q){
    ...
    // sincronizar todos los buscadores (Pendientes y Procesados) y sus contadores
  ...
  function injectSearch(){
    ...
    refs.forEach(function(ref){
      // evitar duplicar: si justo antes de este boton ya hay un buscador, saltear
  ...
  // Por N de turno: rango Hasta=hoy / Desde=179 dias atras (limite 180)
  ...
  // Por DNI/documento: rango ref-1mes / ref+1mes (ref = fecha del pedido)
  ...
  // ---- HIS: mostrar N de pedido medico (Nº Referencia) en el grid de turnos ----
  var PEDIDOMAP={};
  ...
  function annotateHisGrid(){
    ...
    [].slice.call(document.querySelectorAll('tr')).forEach(function(tr){
      ...
      if(!turno) return;
      tr.dataset.fmHis='1';
      // Un turno con varias prestaciones ocupa varias filas: mostrar el cartel UNA sola vez por turno.
  ...
  // Junta los DNIs que aparecen en la pagina de B y los manda al tablero local.
  function collectBDnis(){
    try{
      var txt=document.body.innerText||'';
      var set={}, m, re=/DNI[-\s]?(\d{6,9})/g;
      while((m=re.exec(txt))){ set[m[1]]=1; }
      var dnis=Object.keys(set);
      if(dnis.length) gmPost(SERVER+'/api/bdnis',{dnis:dnis});
    }catch(e){}
  }

  function start(){
    // HIS FUESMEN: si vino un turno en el # de la URL, completarlo y buscar.
    if(/his\.fuesmen\.edu\.ar/i.test(location.host)){
      // El tablero reusa SIEMPRE la misma pestaña (destino 'fuesmenHIS') y cambia el #fuesmenTurno.
      // Procesamos al cargar y tambien cuando cambia el hash (misma pestaña, turno nuevo).
      function runHis(){
        var h=location.hash||'';
        var mt=h.match(/fuesmenTurno=(\d+)/);
        var md=h.match(/fuesmenDni=(\d+)/);
        if(!mt && !md) return;
        // limpiar el hash apenas lo leo, para que el postback de BUSCAR no dispare en bucle
      ...
      runHis();
      // Mostrar N de pedido medico en el grid de turnos del HIS (de la worklist, columna Nº Referencia)
      var cwl=null; try{ cwl=JSON.parse(localStorage.getItem('fuesmen_wl')||'null'); }catch(e){}
      if(cwl && cwl.length){ buildPedidoMap(cwl); annotateHisGrid(); }
      gmGet(SERVER+'/api/worklist', function(l){ try{localStorage.setItem('fuesmen_wl',JSON.stringify(l));}catch(e){} buildPedidoMap(l); annotateHisGrid(); }, function(){});
    ...
    // Solo actuar en la pantalla de pedidos. En otras paginas del Italiano (listados, etc.)
    // el asistente NO hace nada, para no trabarlas.
    if(!/pedido-medico-grupo/i.test(location.pathname)) return;
    function go(list){
      if(!Array.isArray(list)) list=[list];
      // MODO LOTE: no redibujar todos los paneles (lento). Solo barra (para "Detener") y seguir la cola.
      if(queueActive()){ injectToggle(); setTimeout(processQueue, 200); return; }
      buildIndex(list);
      buildPedidoMap(list);   // turno -> Nº Referencia FUESMEN
      buildSafeIndex(list);   // Nº Referencia -> turno (único), para detectar casos seguros
      injectToggle();
      annotate();
      // GeneXus dibuja los pedidos despues de cargar: reintentar anotar enseguida varias veces
      [120,350,800,1600,3000].forEach(function(ms){ setTimeout(annotate, ms); });
      setTimeout(collectBDnis, 2000);
      // Si volvemos de Guardar, saltar automaticamente al proximo verde pendiente.
      ...
      var t1;
      new MutationObserver(function(){ clearTimeout(t1); t1=setTimeout(annotate, 250); }).observe(document.body,{childList:true,subtree:true});
    }
    function cacheGet(){ try{ return JSON.parse(localStorage.getItem('fuesmen_wl')||'null'); }catch(e){ return null; } }
    function cacheSet(l){ try{ localStorage.setItem('fuesmen_wl', JSON.stringify(l)); }catch(e){} }
    // Cache PERSISTENTE (localStorage del Italiano): la worklist queda lista al instante,
    // incluso al abrir una ventana/pestaña nueva. Se refresca en 2do plano.
    var cached=cacheGet();
    if(cached && cached.length){
      go(cached);
      toast('Asistente FUESMEN activo - '+cached.length+' turnos','#0969da');
      gmGet(SERVER+'/api/worklist', function(list){
        cacheSet(list);
        if(Array.isArray(list) && list.length!==cached.length && !queueActive()){
          buildIndex(list);
          buildPedidoMap(list);
          buildSafeIndex(list);
          SIDE_REG={}; SAFE_MAP={};
          [].slice.call(document.querySelectorAll('.fm-panel')).forEach(function(p){ p.remove(); });
          [].slice.call(document.querySelectorAll('[data-fm-done]')).forEach(function(a){ a.removeAttribute('data-fm-done'); });
          annotate();
          toast('Worklist actualizada - '+list.length+' turnos','#0969da');
        }
      }, function(){});
    } else {
      gmGet(SERVER+'/api/worklist', function(list){
        cacheSet(list); go(list);
        toast('Asistente FUESMEN activo - '+(list.length||0)+' turnos cargados','#0969da');
      }, function(){
        toast('No pude leer la worklist. Esta abierto Iniciar-Tablero.bat y generaste la worklist?','#d1242f');
      });
    }
  }
