...
(function () {
  ...
  function revCount(){ var s={}; [].slice.call(document.querySelectorAll('[onclick*="informe("]')).forEach(function(a){ var id=pedidoId(a); if(id && REVISAR[id]) s[id]=1; }); return Object.keys(s).length; }
...
(function () {
  ...
  function buildHTML(P){
    ...
    +'fill(fa,uniq("aseguradora"));fill(fc,uniq("cuenta"));'
    +'var rows=[].slice.call(document.querySelectorAll("#t tbody tr"));'
    +'function flt(){var s=q.value.toLowerCase(),e=fe.value,tp=ft.value,ag=fa.value,cu=fc.value,n=0;rows.forEach(function(r){var ok=(!s||r.textContent.toLowerCase().indexOf(s)>=0)&&(!e||r.getAttribute("data-estado")===e)&&(!tp||r.getAttribute("data-tipo")===tp)&&(!ag||r.getAttribute("data-aseg")===ag)&&(!cu||r.getAttribute("data-cuenta")===cu);r.className=ok?"":"hide";if(ok)n++;});sh.textContent="Mostrando: "+n;}'
    +'q.oninput=flt;fe.onchange=flt;ft.onchange=flt;fa.onchange=flt;fc.onchange=flt;'
...
(function () {
      ...
      try{
        ...
        fetch('pedido-medico-prt.php',{method:'POST',body:body})
          ...
          .then(function(html){
            ...
            var ubic=(t.match(/Ubicaci[oó]n:\s*([^:]{0,60})/i)||[])[1]||'';
            var internado=/piso|sala|cama/i.test(ubic);
            resolve({ mat:mat, os:(os||'').trim(), int:internado });
          })
  ...
  function otOpen(info){
    ...
    fetchDetail(info.ped).then(function(x){
      var pl={ est:info.est, cod:info.cod, dni:info.dni, pac:info.pac, med:info.med, ped:info.ped, mat:(x&&x.mat)||'', os:(x&&x.os)||'', int:!!(x&&x.int) };
  ...
  function matchFinamed(sel){
    var opts=[].slice.call(sel.options), pref=null, any=null;
    opts.forEach(function(o){ if(!o.value) return; var t=norm(o.text);
      if(/FINAMED/.test(t)){ if(!any) any=o; if(/HOSP ITAL/.test(t) || /PROV/.test(t)){ if(!pref) pref=o; } }
    });
    return pref||any;
  }
  function matchCuenta(sel, os){
    var w=norm(os); if(!w) return null;
    var opts=[].slice.call(sel.options), exact=null, part=null, fuzzy=null, fz=0;
    opts.forEach(function(o){ if(!o.value) return; var t=norm(o.text);
      if(t===w) exact=o;
      else if(!part && w.length>=3 && (t.indexOf(w)>=0 || w.indexOf(t)>=0)) part=o;
      var sc=score(os,o.text); if(sc>fz){ fz=sc; fuzzy=o; }
    });
    return exact||part||(fz>=0.5?fuzzy:null);
  }
  function fmFire(el){
  ...
  function fillTurnoForm(p, raw){
    ...
    if(p.int && !raw.tipoDone){
      ...
      radios.forEach(function(r){ if(intR) return;
        ...
        var v=(r.value||'');
        if(v.trim()==='2' || /^INT$/i.test(v.trim()) || /\bINT\b/i.test(v) || /^\s*INT\b/i.test(lab) || /INTERNAD/i.test(lab)) intR=r; // _VMTIPOTURNO: value 2 = INT (verificado 2026-07-30)
      });
